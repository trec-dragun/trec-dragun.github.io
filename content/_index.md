...
Assume there is a (general public) reader who is looking through online news. This track has the following two parallel tasks with the same submission due date. Participants can choose to engage in either one or both tasks. Unlike traditional fact-checking tasks, DRAGUN emphasizes providing readers with more context from multiple sources with the goal to support nuanced and independent judgment, instead of trying to figure out an "absolute truth" and convince readers to believe it.
Unlike traditional fact-checking tasks that often aim to establish an "absolute truth" and present a verdict, DRAGUN focuses on providing readers with multi-source context to foster their own informed judgments.
